...
export const render = async ({ animation, start, end, viewport: { width, height, scale, fps }, out }: RenderOptions) => {
    ...
    for (let curFrame = start; curFrame < end; curFrame += 1) {
        ...
        await page.evaluate(
            (curTime: number) => {
                ...
                animations.forEach((a) => { a.pause(); a.currentTime = curTime; })
            }, curTime
        )

        await page.evaluate(`ANI.tick(${curTime})`)
